---
title: "sec 2-International collaboration shapes"
...
```{r}
mobi_rate_pair <-read.csv("//VUW/Personal$/Homes/G/geh1/Desktop/Project of PhD/previous collaboration and future mobility/Journal paper_2023/mobility ratio--origin and destination from--more than 100--1990-2019_2023.csv") 
```
...
plot_legend <- ggplot(df_top3_long, aes(
  ...
  theme_minimal()+
    ...
    legend.direction = "horizontal",  # 👈 关键设置
...
png(file="C:/Users/geh1/OneDrive - Universiteit Leiden/Desktop/Project of PhD/previous collaboration and future mobility/Journal paper_2023/1b/Fig QSS6--top 3 destination and collaboration.png", width=2000,height=1500)
...
mobi_rate_pair $size[mobi_rate_pair $size < 10] <- 6

```


```{r}
png(file="C:/Users/geh1/OneDrive - Universiteit Leiden/Desktop/Project of PhD/previous collaboration and future mobility/Journal paper_2023/1b/Fig QSS7--mobility rate--origin and detinaiton.png", width=1500,height=1500)
 
mobi_rate_pair_pair <-mobi_rate_pair  %>% arrange(shape, desc(r_mobi)) %>% mutate(size_scale =scales::rescale(size, to = c(5, 50))) %>% arrange((order)) 

ggplot(mobi_rate_pair_pair,
  ...
  #  ) +
    geom_vline(
      xintercept = median(mobi_rate_pair_pair$ MRot_nc),
      linetype = "dashed",
      color = "grey",
      size = 1.2
    ) +
    geom_hline(
      yintercept = median(mobi_rate_pair_pair$MRot_c),
      linetype = "dashed",
      color = "grey",
      size = 1.2
    ) +
      geom_point(
      colour = mobi_rate_pair_pair$color,
      size = mobi_rate_pair_pair$size_scale,
      #size = mobi_rate_pair_p$size,
      shape = mobi_rate_pair_pair$shape
    )+
    #geom_abline(intercept = round(coeff[1],1), slope = round(coeff[2],1), color="blue",
    #            linetype="dotted", size=1.2)+
    geom_text_repel(aes(label=ifelse(mobi_rate_pair_pair$size_scale>13.65 & mobi_rate_pair_pair$mobility_pair!="US-CA" &
                                       mobi_rate_pair_pair$mobility_pair!="US-KR" &
                                       mobi_rate_pair_pair$mobility_pair!="US-UK" 
                                     & mobi_rate_pair_pair$mobility_pair!="FR-US"
                                       & mobi_rate_pair_pair$mobility_pair!="UK-US"
                                     & mobi_rate_pair_pair$mobility_pair!="DE-US"
                                     ,as.character(mobi_rate_pair_pair$mobility_pair),'')),hjust=0.5,
              vjust=0.5,size=8)+
    geom_text(aes(label = ifelse(mobi_rate_pair_pair$mobility_pair == "US-CA", 
                                   "US-CA", '')),  hjust = 0, vjust = 1, size = 8)+
    geom_text(aes(label = ifelse(mobi_rate_pair_pair$mobility_pair == "US-KR", 
                                   "US-KR", '')),  hjust = 1, vjust = 0, size = 8)+
    geom_text(aes(label = ifelse(mobi_rate_pair_pair$mobility_pair == "US-UK", 
                                   "US-UK", '')),  hjust =0, vjust = 1, size = 8)+
      geom_text(aes(label = ifelse(mobi_rate_pair_pair$mobility_pair == "US-CN", 
                                   "US-CN", '')),  hjust =0.5, vjust = 0.5, size = 8)+
   geom_text(aes(label = ifelse(mobi_rate_pair_pair$mobility_pair == "US-DE", 
                                   "US-DE", '')),  hjust =1, vjust = 0, size = 8)+
  geom_text(aes(label = ifelse(mobi_rate_pair_pair$mobility_pair == "FR-US", 
                                   "FR-US", '')),  hjust =0, vjust = 1, size = 8)+
   geom_text(aes(label = ifelse(mobi_rate_pair_pair$mobility_pair == "UK-US", 
                                   "UK-US", '')),  hjust =0, vjust = 1, size = 8)+
   geom_text(aes(label = ifelse(mobi_rate_pair_pair$mobility_pair == "DE-US", 
                                   "DE-US", '')),  hjust =0.5, vjust = 0, size = 8)+
  

   # geom_text(aes(label=ifelse(mobi_rate_pair_p$size_scale>10,as.character(mobi_rate_pair_p$mobility_pair),'')),hjust=0,vjust=0,size=10)+
...
```{r}
png(file="C:/Users/geh1/OneDrive - Universiteit Leiden/Desktop/Project of PhD/previous collaboration and future mobility/Journal paper_2023/1b/Fig QSS8--countries--mobility rate--origin and detinaiton_2023.png", width=1500,height=1400)
...
```{r}
# 1. Remove rows with missing region names
mobi_rate_pair_region_no_NA<- mobi_rate_pair %>%
  filter(region_origin != '#N/A' & region_destination != '#N/A')

# 2. Aggregate mobility and collaboration counts by origin-destination region pairs
...
# 3. Compute Gini coefficients to assess distribution inequality
gini_coll_region <- ineq::Gini(mobi_coll_region$sum_coll)
#0.7486486
gini_mobi_region <- ineq::Gini(mobi_coll_region$sum_mobi)
#0.7265775

# 4. define region color mapping (for plotting later)
...
# 5. Extract mobility data: region-to-region mobility counts only
n_mobility_region<-mobi_coll_region[,c("region_origin","region_destination","sum_mobi")] 

# 6. Extract collaboration data: region-to-region collaboration counts only
n_collaboration_region<-mobi_coll_region[,c("region_origin","region_destination","sum_coll")] 

# 7. Draw mobility and collaboration chord diagram across regions
png(file="C:/Users/geh1/OneDrive - Universiteit Leiden/Desktop/Project of PhD/previous collaboration and future mobility/Journal paper_2023/1b/Fig QSS--A3--mobility and collaboration--chord diagram between regions.png", width=3300,height=1600,res=300)

# 7.1.  Set 2-panel layout: one for mobility, one for collaboration

par(mfrow=c(1,2))

# 7.2 Draw directional mobility chord diagram
circos.clear()
...
# 7.3. Draw collaboration chord diagram 
...
chordDiagram(n_collaboration_region, directional = TRUE,grid.col=region_color,
             ...
             #grid =list( fontsize = 12,fontfamily="Arial")
             link.sort=TRUE,
             link.decreasing=TRUE,
              direction.type = c("diffHeight", "arrows"),
    link.arr.type = "big.arrow",
...
gini_coll_origin_region
gini_mobi_origin_region
...
```{r}
# 1. Define region labels and abbreviations once for reuse
...
# 2. Convert region names to factor levels with abbreviations
...
# 3. Calculate average mobility rates between each origin–destination region pair
...
# 4. Reshape from wide to long format for plotting
...
# 5. Plot: Line chart of mobility rates by destination region, faceted by origin

plot_MRotnc_region<-ggplot(CMRot_region_long, aes(x = factor(region_destination), y = value, color = variable)) +
  geom_line(aes(group = variable), size = 4) +
  geom_point(size = 4) +
  scale_color_manual(
    values = c("avg_MRot_nc" = "#d62728", "avg_MRot_c" = "#1f77b4", "avg_MRot_all" = "#2ca02c"),
    labels = c("avg_MRot_nc" = "ave. MRot_nc", "avg_MRot_c" = "ave. MRot_c", "avg_MRot_all" = "ave. MRot_all")
  ) +
  scale_y_continuous(
    name = "Region of origin country", limits = c(0, 0.11), expand = c(0, 0), breaks = c(0, 0.02, 0.04, 0.06, 0.08, 0.1),
    labels = c("0", "", "", "0.6", "", "")
  ) +
  facet_grid(rows = vars(region_origin), scales = "free_y", switch = "y") +  # ??????????????????facet,????????????????????????
...
# 6. Plot: Bar chart of average CMRot by origin, faceted by destination

plot_CMRot_region<-ggplot(CMRot_region, aes(x = factor(region_origin, levels = c("SSA", "SA", "NA", "MENA","LAC","ECA", "EAP")))) +
  geom_bar(aes(y = avg_CMRot, fill = region_origin), stat = "identity", position = position_dodge(width = 0.8)) +
 geom_text(aes(y = avg_CMRot, 
                label = round(avg_CMRot, 0), 
               hjust = ifelse(avg_CMRot < 700, -0.4, 1.1)), 

             size = 15) +  
  scale_y_continuous(breaks = seq(0, 730, by = 200),  
                     labels = c("0",  "", "400", "")) + 
  scale_fill_manual(values = c( 
    ...
    "SA" = "#8c564b",
    "SSA" = "#e377c2")) +
facet_grid(. ~ region_destination, scales = "free_y", space = "free_x"  , switch = "y" ) +  
  ...
  theme(
    ...
    axis.title.y.right = element_text(margin = margin(l = 20)),
    legend.position = "none",
    strip.text = element_text(size = 50, hjust = 0.5),  
    strip.background = element_blank() 
  )+
  xlab("Region of origin country") +  
  ylab("Average CMRot") +
  labs(subtitle = "Region group of destination country") 

# 7. Combine both plots side by side and save to file
grid_layout <- rbind(
  c(1, 1,  2, 2),
  c(1, 1,  2, 2)
)
...
png(file="C:/Users/geh1/OneDrive - Universiteit Leiden/Desktop/Project of PhD/previous collaboration and future mobility/Journal paper_2023/1b/Fig QSS9--mobility rate_2023.png", width=3000,height=1500) 
...
```{r}
# 1. Remove rows with missing income group data
mobi_rate_pair_income_no_NA <- mobi_rate_pair %>%
  filter(income_origin != '#N/A' & income_destination != '#N/A')

# 2. Aggregate mobility and collaboration numbers by income group pairs
...
# 4. Define income group colors
income_color<-c("High"="#1F77B4", 
                   "Upper middle"="#FF7F0E", 
                   "Lower middle"="#2CA02C", 
                   "Low"="#D62728")
# 5. Extract mobility data: income group-to-income group mobility counts only

n_mobility_income<-mobi_coll_income[,c("income_origin","income_destination","sum_mobi")] 

# 6. Extract mobility data: income group-to-income group collaboration counts only

n_collaboration_income<-mobi_coll_income[,c("income_origin","income_destination","sum_coll")] 

# 7. Draw mobility and collaboration chord diagram across income group
png(file="C:/Users/geh1/OneDrive - Universiteit Leiden/Desktop/Project of PhD/previous collaboration and future mobility/Journal paper_2023/1b/Fig QSS--A4--mobility and collaboration--chord diagram between income.png", width=3300,height=1600,res=300)
...
# 7.3. Draw directional collaboration chord diagram                
circos.clear()
circos.par(gap.after = 8,  # ?????????????????????????????????
           track.height = 0.01,  # ?????????????????????
           circle.margin = c(0.13, 0.13, 0.13, 0.13))  # ??????????????????
...
gini_mobi_origin_income 
gini_coll_origin_income 
...
```{r}
# 1. Define income group labels and abbreviations
income_levels<-c("High", "Upper middle", "Lower middle", "Low")
income_labels= c("H", "UM", "LM", "L")

# 2. Convert income group names to factor levels with abbreviations
...
# 3. Calculate average mobility rates between each origin–destination income group pair
...
# 5. Line chart of three mobility rates by income group of destination, faceted by income group of origin


plot_MRotnc_income<-ggplot(CMRot_income_long,  aes(x = factor(income_destination), y = value, color = variable)) +
  geom_line(aes(group = variable), size = 4) +
  geom_point(size = 4) +
  scale_color_manual(
    values = c("avg_MRot_nc" = "#d62728", "avg_MRot_c" = "#1f77b4", "avg_MRot_all" = "#2ca02c"),
  ...
  scale_y_continuous(
    name = "Income group of origin country", limits = c(0, 0.11), expand = c(0, 0), breaks = c(0, 0.02, 0.04, 0.06, 0.08, 0.1),
    labels = c("0", "", "", "0.6", "", "")
  ) +
...
# 6. Bar chart of average CMRot by income group of destination, faceted by income group of origin

plot_CMRot_income<-ggplot(CMRot_income, aes(x = factor(income_origin, levels = c("L", "LM", "UM", "H"),labels=c("L", "LM", "UM","H")))) +
  geom_bar(aes(y = avg_CMRot, fill = income_origin), stat = "identity", position = position_dodge(width = 0.8)) +
 geom_text(aes(y = avg_CMRot, 
                label = round(avg_CMRot, 0), 
                #vjust = ifelse(CMRot < 700, 0, 0),
               hjust = ifelse(avg_CMRot < 500, -0.4, 1.1)), # ??????CMRot????????????vjust
           #
             size = 15) +  # ????????????bar??????nudge_y
  scale_fill_manual(values = c( 
    "H" = "#1f77b4",
    ...
    "LM"= "#2ca02c",
    "L" = "#d62728")) +
facet_grid(. ~ income_destination, scales = "free_y", space = "free_x", switch = "y"
           ) +  # ??????????????????
 scale_y_continuous(limits = c(0, 580), breaks = c(0, 200, 400)) +
  coord_flip() + 
  theme_bw() +
  theme(
    panel.grid.major = element_blank(),
...
xlab("Income group of origin country") +  # ?????? x ?????????
  ylab("Average CMRot") + # ?????? y ?????????
  labs(subtitle = "Income group of destination country") 
...
png(file="C:/Users/geh1/OneDrive - Universiteit Leiden/Desktop/Project of PhD/previous collaboration and future mobility/Journal paper_2023/1b/Fig QSS10--mobility rate_2023.png", width=3000,height=1500) 
